...
WinNotifier に接続できない場合（名前解決失敗、接続拒否、タイムアウト）、winnotify-send はエラーを抑制して正常終了します。LAN 外に持ち出した場合などに通知の失敗でスクリプトが中断するのを防ぎます。

認証エラーなどの HTTP エラーは抑制されません。

`--debug` オプションを付けると接続エラーも抑制せずそのまま返します。
